...
export const NewScan: React.FC<{ onStartScan?: (newCase: Scan) => void }> = ({ onStartScan }) => {
        ...
        if (activeStep === 1) {
            ...
            try {
                ...
                const newCase: Scan = {
                    uuid: 'pending',
                    id: 'pending...',
                    name: selectedFile ? selectedFile.name : 'New Investigation',
                    status: 'pending',
    ...
    return (
        <div className="max-w-5xl mx-auto pt-8">
            {/* Modern Stepper */}
            <div className="flex justify-between items-start mb-12 px-12 relative">
                {/* Connecting Line */}
                <div className="absolute top-7 left-12 right-12 h-0.5 bg-slate-800 -z-10 -translate-y-1/2">
                    <div
            ...
            <div className="bg-[#13111c]/60 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/5 overflow-hidden relative">
                ...
                <div className="p-10">
                    <h2 className="text-3xl font-bold text-white mb-8 text-center">{steps[activeStep - 1].label}</h2>

                    <form onSubmit={handleNext}>
                        {activeStep === 1 && (
                            <div className="border border-dashed border-white/10 rounded-2xl p-16 flex flex-col items-center justify-center text-center hover:border-primary/50 hover:bg-white/[0.02] transition-all duration-300 cursor-pointer group bg-black/20" onClick={triggerFileInput}>
                                <input
                                    type="file"
                                    id="file-upload"
                                    className="hidden"
                                ...
                                />
                                <div className="w-24 h-24 bg-gradient-to-br from-primary/10 to-secondary/10 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 shadow-lg shadow-purple-500/10 border border-white/5">
                                    <HardDrive className="w-10 h-10 text-primary drop-shadow-[0_0_10px_rgba(168,85,247,0.5)]" />
                                </div>
                                <h3 className="text-xl font-bold text-white">{selectedFile ? selectedFile.name : 'Upload Memory Dump'}</h3>
                                <p className="text-slate-400 mt-2 text-sm max-w-sm">{selectedFile ? `Size: ${(selectedFile.size / (1024 * 1024)).toFixed(2)} MB` : "Drag and drop your .raw, .dmp, or .mem file here. We'll automatically calculate the hash."}</p>
                                ...
                                {uploading && (
                                    <div className="mt-6 flex flex-col items-center animate-fadeIn">
                                        ...
                                        <p className="text-xs text-slate-400 font-medium">Uploading to Secure Storage...</p>
                                    </div>
                                )}

                                <button type="button" className="mt-8 px-8 py-3 bg-white text-black rounded-lg text-sm font-bold hover:bg-slate-200 transition-colors shadow-lg hover:shadow-xl hover:-translate-y-0.5 transform duration-300" onClick={(e) => { e.stopPropagation(); triggerFileInput(); }}>
                        ...
                        {activeStep === 2 && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                <div className="space-y-6">
                                    <label className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2 block ml-1">Analysis Engine</label>
                                    <div className="grid grid-cols-2 gap-4">
                                        <label className="cursor-pointer group">
                                            ...
                                            />
                                            <div className="p-5 rounded-xl border border-white/10 bg-black/20 peer-checked:border-primary peer-checked:bg-primary/10 transition-all flex flex-col items-center text-center h-full hover:border-white/30 hover:bg-white/5 relative overflow-hidden">
                                                ...
                                                <span className="font-bold text-white mb-1 relative z-10">Volatility 3</span>
                                        ...
                                        <label className="cursor-pointer group">
                                            ...
                                            <div className="p-5 rounded-xl border border-white/10 bg-black/20 peer-checked:border-primary peer-checked:bg-primary/10 transition-all flex flex-col items-center text-center h-full hover:border-white/30 hover:bg-white/5 relative overflow-hidden">
                                                ...
                                                <span className="font-bold text-white mb-1 relative z-10">Volatility 2</span>
                                    ...
                                    {engine === 'vol2' && (
                                        <div className="relative animate-fadeIn">
                                            <label className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2 block ml-1">Target Profile</label>
                                            <div className="flex items-center bg-black/20 border border-white/10 rounded-xl px-4 py-4 hover:border-primary/50 transition-colors group focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/50">
                                                <Cpu className="w-5 h-5 text-slate-500 group-hover:text-primary transition-colors mr-4" />
                                                <select
                                                    className="w-full bg-transparent text-white focus:outline-none appearance-none font-medium [&>option]:bg-[#13111c]"
                                ...
                                <div className="space-y-6">
                                    <div className="relative">
                                        <label className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2 block ml-1">Operating System</label>
                                        <div className="flex items-center bg-black/20 border border-white/10 rounded-xl px-4 py-4 hover:border-primary/50 transition-colors group focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/50">
                                            <Cpu className="w-5 h-5 text-slate-500 group-hover:text-primary transition-colors mr-4" />
                                            <select
                                                className="w-full bg-transparent text-white focus:outline-none appearance-none font-medium [&>option]:bg-[#13111c]"
                                    ...
                                    <div className="relative">
                                        <label className="text-slate-400 text-xs font-bold uppercase tracking-wider mb-2 block ml-1">Docker Image</label>
                                        <div className={`flex items-center bg-black/20 border border-white/10 rounded-xl px-4 py-4 transition-colors group focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/50 ${imageError ? 'border-red-500/50' : 'hover:border-primary/50'}`}>
                        ...
                        {activeStep === 3 && (
                            <div className="space-y-6">
                                ...
                                <div className="bg-black/20 rounded-xl p-8 border border-white/10 space-y-5 text-sm font-medium">
                                    <div className="flex justify-between items-center border-b border-white/5 pb-5">
                                        ...
                                        <span className="text-white font-mono bg-white/5 border border-white/10 px-3 py-1.5 rounded text-xs flex items-center">
                                            <HardDrive className="w-3 h-3 mr-2 text-slate-400" />
                                    ...
                                    <div className="flex justify-between items-center border-b border-white/5 pb-5">
                                        ...
                                        <div className="text-right">
                                            <div className="text-white">{osType === 'auto' ? 'Auto-Detect OS' : osType}</div>
                                            <div className="text-xs text-slate-500 mt-1">{dockerImage}</div>
                                        </div>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-slate-500">Engine</span>
                                        <span className="text-white flex items-center bg-primary/10 border border-primary/20 px-3 py-1 rounded-full text-xs font-bold text-primary">
                                            {engine === 'vol3' ? <Zap className="w-3 h-3 mr-1" /> : <Server className="w-3 h-3 mr-1" />}
