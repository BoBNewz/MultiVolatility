...
class TestCreateScan:
    def test_missing_dump_returns_400(self, client, auth_headers):
        resp = client.post(
            "/scan", json={"os": "windows", "mode": "vol3"}, headers=auth_headers
        )
    ...
    def test_missing_os_returns_400(self, client, auth_headers):
        resp = client.post(
            "/scan",
            json={"dump": "/some/file.dmp", "mode": "vol3"},
...
class TestCreateScanReturnsScanId:
    def test_returns_uuid(self, client, auth_headers, tmp_file):
        ...
        assert uuid_pattern.match(data["scan_id"]), (
            f"scan_id not a UUID: {data['scan_id']}"
        )
...
class TestErrorPaths:
    ...
    def test_scan_invalid_mode_returns_400(self, client, auth_headers, tmp_file):
        resp = client.post(
            "/scan",
            json={"dump": tmp_file, "mode": "invalid", "linux": True},
            headers=auth_headers,
        )
        assert resp.status_code == 400
    ...
    def test_scan_both_os_flags_returns_400(self, client, auth_headers, tmp_file):
        resp = client.post(
            "/scan",
            json={"dump": tmp_file, "mode": "vol3", "linux": True, "windows": True},
            headers=auth_headers,
        )
        assert resp.status_code == 400
    ...
    def test_scan_neither_os_flag_returns_400(self, client, auth_headers, tmp_file):
        resp = client.post(
            "/scan", json={"dump": tmp_file, "mode": "vol3"}, headers=auth_headers
        )
        assert resp.status_code == 400
