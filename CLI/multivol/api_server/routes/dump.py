...
def _build_vol_dump_command(
    ...
    if scan["os"] == "linux" and config.get("fetch_symbol"):
        cmd.extend(["--remote-isf-url", VOLATILITY3_SYMBOLS_BANNER_URL])
...
def _run_docker_vol(
    ...
    """Run a Volatility Docker container, auto-pulling the image if missing.
...
def _persist_task_result(
    ...
    if status == "completed" and created_files:
        ...
        c.execute(
            "UPDATE dump_tasks SET status = 'completed', output_path = ? WHERE task_id = ?",
            (output_path, task_id),
        )
    else:
        c.execute(
            "UPDATE dump_tasks SET status = 'failed', error = ? WHERE task_id = ?",
            (error_msg, task_id),
        )
    conn.commit()
    conn.close()
    ...
    try:
        ...
        if scan.get("config_json"):
            try:
                ...
                logging.warning(
                    "Failed to parse config_json for scan %s",
                    scan.get("uuid", "?"),
                    exc_info=True,
                )
        ...
        if not os.path.isabs(uploaded_path):
            uploaded_path = os.path.join(STORAGE_DIR, uploaded_path)
        volumes = {
            resolve_host_path(os.path.dirname(uploaded_path)): {
                "bind": "/dump_dir",
                "mode": "ro",
            },
            resolve_host_path(task_out_dir): {"bind": "/output", "mode": "rw"},
            resolve_host_path(symbols_path): {"bind": "/symbols", "mode": "rw"},
            resolve_host_path(cache_path): {
                "bind": "/root/.cache/volatility3",
                "mode": "rw",
            },
        }
...
def dump_file_from_memory(scan_id: str) -> Response:
    ...
    default_image = (
        "sp00kyskelet0n/volatility3"
        if scan["volatility_version"] != "2"
        else "sp00kyskelet0n/volatility2"
    )
    ...
    c.execute(
        "INSERT INTO dump_tasks (task_id, scan_id, status, created_at) VALUES (?, ?, ?, ?)",
    ...
    thread = threading.Thread(
        target=background_dump_task,
        args=(task_id, scan_dict, virt_addr, image, file_path),
    )
    thread.daemon = True
    thread.start()
